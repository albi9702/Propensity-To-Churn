...
* `COD_FI`: identify the fidelty program;
...
knitr::kable(head(fidelity))
knitr::kable(summary(fidelity))
```
...
knitr::kable(head(account))
knitr::kable(summary(account))
```
...
knitr::kable(head(address))
knitr::kable(summary(address))
```
...
knitr::kable(head(privacy))
knitr::kable(summary(privacy))
```
...
knitr::kable(head(campaign_category))
knitr::kable(summary(campaign_category))
```
...
knitr::kable(head(campaign_event))
knitr::kable(summary(campaign_event))
```
...
knitr::kable(head(tickets))
knitr::kable(summary(tickets))
```
...
dist_num_fid_x_cli <- num_fid_x_cli %>%
                        group_by(NUM_FIDs, NUM_DATEs)            %>% #-- Group by Client and Date of Activation
...
```{r Info Combined Fidelity}
client_fidelity_first <- fidelity_clean                        %>%
                           group_by(ID_CLI)                    %>% #-- Grouped by minimun activity
...
account_clean <- account_clean %>%
                  ...
                  mutate(EMAIL_PROVIDER = fct_explicit_na(EMAIL_PROVIDER, "(missing)")) %>% #-- NA with Categorial
