...
#-- non può essere messo in rel con la next purchase curve perchè nel dataset tickets_clean uno stesso cliente è considerato nel momento in cui ritorna, mentre in questo caso viene considerato una sola volta (35% vs 75%)
```
...
tree <- rpart(CHURN ~ RECENCY + SPESA + TOT_PURCHASE + REGION + LAST_COD_FID + TYP_JOB,
              data = train)
...
```{r Logistic}
#Logistic
logistic <- glm(CHURN ~ RECENCY + SPESA + TOT_PURCHASE +
                  REGION + LAST_COD_FID + TYP_JOB,
...
SVM <- train(CHURN ~ RECENCY + SPESA + TOT_PURCHASE +
                 REGION + LAST_COD_FID + TYP_JOB, data = train, 
                 method = 'svmLinear',
